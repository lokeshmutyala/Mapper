...
public class AdapterClassHyd extends RecyclerView.Adapter<AdapterClassHyd.MyViewHolder>{
    private final String TAG="adapterclasshyd";
    ...
    int selecteditem=-1;
    Context context;
    public AdapterClassHyd(List<StoreData> storeDataList , Context context,Application application) {
    ...
    @Override
    public AdapterClassHyd.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View itemView= LayoutInflater.from(parent.getContext()).inflate(R.layout.single_row,parent,false);
        return new AdapterClassHyd.MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(final AdapterClassHyd.MyViewHolder holder, final int position) {
...
//                            context.startActivity(intent);
                    ((HydAuditPage)context).performAction(storeDataList.get(position));
//                    }else {
